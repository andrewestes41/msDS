...
Now, given a particular fold, `i`, find the RMSE on that fold.

```{r}
i <- 1
fit <- lm(Price ~ Mileage, data=saturn[folds != i, ])
...
rmse.vec <- numeric(k)
for(i in 1:k){
  fit <- loess(Price ~ Mileage, data=saturn[folds != i, ], degree=1, span=0.5)
  # Note: na.rm=TRUE is a quick fix because loess won't predict outside the
  # range of training data.
  rmse.vec[i] <-
    RMSE(saturn$Price[folds==i], predict(fit, newdata=saturn[folds==i, ]), na.rm=TRUE)
}
rmse.vec
mean(rmse.vec)
```
...
rmse.saturn <- function(method=lm, seed=23948, k=10, ...){
  folds <- rep(1:k, each=ceiling(nrow(saturn)/k))
  folds <- sample(folds, nrow(saturn))
  rmse.vec <- numeric(k)
  for(i in 1:k){
...
rmse.saturn(lm)
rmse.saturn(lm, seed=22)
rmse.saturn(loess, degree=1, span=0.5)
```
...
rmse.vec <- map_dbl(span.vec,
                    ~rmse.saturn(method=loess, degree=1, span=.x, seed=seed))
...
#
# Note: modelLookup() can help you remember model names provided by `caret`.
loess.train <- train(
